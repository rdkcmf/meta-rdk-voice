...
SRC_URI += " \
        ...
        ${@bb.utils.contains('DISTRO_FEATURES', 'enable-rdkv-bt-voice', 'file://0004-Initialize-AVS-after-residentapp.patch;apply=no', '', d)} \
...
do_apply_patch() {
    ...
    if [ ! -e patch_applied ]; then
        if [ "${@bb.utils.contains('DISTRO_FEATURES', 'enable-rdkv-bt-voice', 'true', 'false', d)}" = "true" ]; then
                ...
                patch -p1 < ${WORKDIR}/0004-Initialize-AVS-after-residentapp.patch
        fi
        touch patch_applied
    fi
}
